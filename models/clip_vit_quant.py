...
__all__ = [
    ...
    'vit_large_patch14_224','vit_large_patch14_336'
]
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = (
            ...
            qkv[1],
            qkv[2],
        )  # make torchscript happy (cannot use tensor as tuple) [B,self.num_heads ,N, C // self.num_heads ]
        attn = (q @ k.transpose(-2, -1)) * self.scale # [B, self.num_heads ,N, C // self.num_heads ] * [B, self.num_heads , C // self.num_heads,N] = [B, self.num_heads , N,N]
        attn = self.qact_attn1(attn)
        attn = self.log_int_softmax(attn, self.qact_attn1.quantizer.scale)
        attn = self.attn_drop(attn)
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # [B, self.num_heads , N,N] * [B, self.num_heads ,N, C // self.num_heads ] = [B, self.num_heads ,N, C // self.num_heads ] -> [B, self.num_heads , C // self.num_heads, N ] -> [B, N, C]
        x = self.qact2(x)
        x = self.proj(x)
        x = self.qact3(x)
        x = self.proj_drop(x)
...
class Block(nn.Module):
    ...
    def forward(self, x, last_quantizer=None):
        x = self.qact2(x + self.drop_path(
            self.attn(
                self.qact1(self.norm1(x, last_quantizer, # post layer-norm activation
                                      self.qact1.quantizer)))))
        x = self.qact4(x + self.drop_path(
            self.mlp(
                self.qact3( # post layer-norm activation
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x) # [*, grid ** 2, width],
        ...
        x = self.qact_embed(x)
        x = x + self.qact_pos(self.pos_embed) # [1, num_patches + 1, embed_dim] = [*, grid ** 2 + 1, width] 
        x = self.qact1(x)
...
def vit_base_patch32_224(pretrained=False,
    ...
    model = VisionTransformer(patch_size=32,
                              ...
                              mlp_ratio=4,
                              qkv_bias=True,
                              norm_layer=partial(QIntLayerNorm, eps=1e-6), # to read
                              quant=quant,
                              calibrate=calibrate,
                              input_quant=False,
                              cfg=cfg,
    ...
    if pretrained:
        url = 'https://storage.googleapis.com/vit_models/augreg/' + \
            'B_32-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_224.npz'
...
def vit_large_patch14_224(pretrained=False,
                          quant=False,
                          calibrate=False,
                          cfg=None,
                          **kwargs):
    ...
    if pretrained:
        url = 'https://storage.googleapis.com/vit_models/augreg/' + \
            'L_14-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.1-sd_0.1--imagenet2012-steps_20k-lr_0.01-res_224.npz'

        load_weights_from_npz(model, url, check_hash=True)
    return model

def vit_large_patch14_336(pretrained=False,
                          quant=False,
                          calibrate=False,
                          cfg=None,
                          **kwargs):
    ...
    if pretrained:
        url = 'https://storage.googleapis.com/vit_models/augreg/' + \
            'L_14-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.1-sd_0.1--imagenet2012-steps_20k-lr_0.01-res_336.npz'
