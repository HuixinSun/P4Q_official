...
class Mlp(nn.Module):
    ...
    def __init__(self,
        ...
        self.c_proj = QLinear(hidden_features,
                           out_features,
                           quant=quant,
                           calibrate=calibrate,
                           bit_type=cfg.BIT_TYPE_W,
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        if norm_layer:
            self.qact_before_norm = QAct(
                ...
                observer_str=cfg.OBSERVER_A,
                quantizer_str=cfg.QUANTIZER_A)
            self.norm = norm_layer(embed_dim) # to read
            ...
            # deafults here
            self.qact_before_norm = nn.Identity() # Identity() is empty operation
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2) # [*, width=768, grid, grid] -> [*, width, grid ** 2] -> [*, grid ** 2, width], quant enabled
